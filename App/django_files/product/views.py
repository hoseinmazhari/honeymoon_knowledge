    ...
    if request.method == 'POST':
        ...
        if action == "run":
            ...
            # if True:
                ...
                final_result = tsksl_s(atk_s.task_name.set_order_point,df_product_list)
                # for i in range(10):
...
def obsolete(request):
    ...
    if request.method == 'POST':
        ...
        if action == "run":
            ...
            # if True:
                ...
                # df_invoices.drop_duplicates(subset=asts.tjCol.mobile, inplace=True)
                # args_ = scales#type: ignore
                final_result = tsksl_s(atk_s.task_name.obsolete,df_product_list)
                # for i in range(10):
                #     print("run", final_result)
                # df = pd.read_excel("")
            # driver.get('http://aradpayamak.net')
    ...
    return render(request, 'product/obsolete_products.html',{'form': form, "result":"stoped"})

def correct_zero_prices_barcodes(request):
    form = ExcelProduct_list()
    result = "stoped"
    # for i in range(10):
    #     print("start")
    ...
    if request.method == 'POST':
        ...
        if action == "run":
            ...
            # if True:
                ...
                # df_invoices.drop_duplicates(subset=asts.tjCol.mobile, inplace=True)
                # args_ = scales#type: ignore
                final_result = tsksl_s(atk_s.task_name.correct_zero_prices_barcodes,df_product_list)
                # for i in range(10):
                #     print("run", final_result)
                # df = pd.read_excel("")
            # driver.get('http://aradpayamak.net')
    ...
    return render(request, 'product/correct_zero_prices_barcodes.html',{'form': form, "result":"stoped"})
